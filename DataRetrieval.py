...
def run_singles():
    ...
    p6 = p("06")
    p6_clust, p6_acc ,p6_rand = p6.cluster()
    accuracies.append(p6_acc)
    rands.append(p6_rand)

    p7 = p("07")
    p7_clust, p7_acc, p7_rand = p7.cluster()
    accuracies.append(p7_acc)
    rands.append(p7_rand)

    p8 = p("08")
    p8_clust, p8_acc, p8_rand = p8.cluster()
    accuracies.append(p8_acc)
    rands.append(p8_rand)

    p9 = p("09")
    p9_clust, p9_acc, p9_rand = p9.cluster()
    accuracies.append(p9_acc)
    rands.append(p9_rand)

    p10 = p("10")
    p10_clust, p10_acc, p10_rand = p10.cluster()
    accuracies.append(p10_acc)
    rands.append(p10_rand)

    p11 = p("11")
    p11_clust, p11_acc, p11_rand = p11.cluster()
    accuracies.append(p11_acc)
    rands.append(p11_rand)

    p12 = p("12")
    p12_clust, p12_acc, p12_rand = p12.cluster()
    accuracies.append(p12_acc)
    rands.append(p12_rand)

    p13 = p("13")
    p13_clust, p13_acc, p13_rand = p13.cluster()
    accuracies.append(p13_acc)
    rands.append(p13_rand)

    p14 = p("14")
    p14_clust, p14_acc, p14_rand = p14.cluster()
    accuracies.append(p14_acc)
    rands.append(p14_rand)

    p15 = p("15")
    p15_clust, p15_acc, p15_rand = p15.cluster()
    accuracies.append(p15_acc)
    rands.append(p15_rand)

    p16 = p("16")
    p16_clust, p16_acc, p16_rand = p16.cluster()
    accuracies.append(p16_acc)
    rands.append(p16_rand)

    p17 = p("17")
    p17_clust, p17_acc, p17_rand = p17.cluster()
    accuracies.append(p17_acc)
    rands.append(p17_rand)

    p18 = p("18")
    p18_clust, p18_acc, p18_rand = p18.cluster()
    accuracies.append(p18_acc)
    rands.append(p18_rand)

    p19 = p("19")
    p19_clust, p19_acc, p19_rand = p19.cluster()
    accuracies.append(p19_acc)
    rands.append(p19_rand)

    p20 = p("20")
    p20_clust, p20_acc, p20_rand = p20.cluster()
    accuracies.append(p20_acc)
    rands.append(p20_rand)

    p21 = p("21")
    p21_clust, p21_acc, p21_rand = p21.cluster()
    accuracies.append(p21_acc)
    rands.append(p21_rand)

    p22 = p("22")
    p14_clust, p22_acc, p22_rand = p22.cluster()
    accuracies.append(p22_acc)
    rands.append(p22_rand)

    p23 = p("23")
    p23_clust, p23_acc, p23_rand = p23.cluster()
    accuracies.append(p23_acc)
    rands.append(p23_rand)

    p24 = p("24")
    p24_clust, p24_acc, p24_rand = p24.cluster()
    accuracies.append(p24_acc)
    rands.append(p24_rand)

    p25 = p("25")
    p25_clust, p25_acc, p25_rand = p25.cluster()
    accuracies.append(p25_acc)
    rands.append(p25_rand)

    p26 = p("26")
    p26_clust, p26_acc, p26_rand = p26.cluster()
    accuracies.append(p26_acc)
    rands.append(p26_rand)

    p27 = p("27")
    p27_clust, p27_acc, p27_rand = p27.cluster()
    accuracies.append(p27_acc)
    rands.append(p27_rand)

    p28 = p("28")
    p28_clust, p28_acc, p28_rand = p28.cluster()
    accuracies.append(p28_acc)
    rands.append(p28_rand)

    p29 = p("29")
    p29_clust, p29_acc, p29_rand = p29.cluster()
    accuracies.append(p29_acc)
    rands.append(p29_rand)

    p30 = p("30")
    p30_clust, p30_acc, p30_rand = p30.cluster()
    accuracies.append(p30_acc)
    rands.append(p30_rand)
...
def run_channels(individuals):
    ...
    print("C1 dim: ", np.array(c1_array).shape)
    print("C1 labels: ", c1_labels)
    c1 = p("c1", fv=c1_array, labels=c1_labels)
    c1_clust, c1_acc, _ = c1.cluster()
    ...
    c10_clust, c10_acc, _ = c10.cluster()
    ...
    c11_clust, c11_acc, _ = c11.cluster()
    ...
    c12_clust, c12_acc, _ = c12.cluster()
    ...
    c13_clust, c13_acc, _ = c13.cluster()
    ...
    c14_clust, c14_acc, _ = c14.cluster()
    ...
    c15_clust, c15_acc, _ = c15.cluster()
    ...
    c16_clust, c16_acc, _ = c16.cluster()
    ...
    c17_clust, c17_acc, _ = c17.cluster()
    ...
    c18_clust, c18_acc, _ = c18.cluster()
    ...
    c19_clust, c19_acc, _ = c19.cluster()
    ...
    c20_clust, c20_acc, _ = c20.cluster()
...
def get_all_participants(participants):
    ...
    for p in participants:
        for trial in range(0, len(p.fv)):
            ...
            p_reduced_array.append(p.fv_reduced[trial, :])
        for elem in p.labels:
            labels_array.append(elem)

    return np.array(p_array), np.array(p_reduced_array), np.array(labels_array)